...
| 物料 | 型号 | 数量 | 参考价格 |
|------|------|------|----------|
| 主控板 | ESP32-C3 SuperMini | 1 | ~15元 |
| 显示屏 | 0.96寸 OLED (SH1106, I2C, 128x64) | 1 | ~12元 |
| 旋转编码器 | EC11 (带按键) | 1 | ~2元 |
| 轻触按键 | 6x6x5mm | 2 | ~0.5元 |
| 面包板/洞洞板 | — | 1 | ~5元 |
| 杜邦线 | 若干 | — | ~2元 |
...
2. **LED 闪烁** — 正在尝试连接 WiFi
3. **静默离线** — 首次没有 WiFi 凭据，设备会自动跳过连接，进入离线模式

> ⚠️ 首次开机**不会自动弹出配网页面**，需要手动进入。

### 第三步：配网（连接 WiFi）

1. **唤醒设备** — 按 `GP3` 唤醒键（或旋转旋钮唤醒）
...
1. **LED 常亮** — 表示 WiFi 已连接
2. **自动查询** — 设备会自动查询电费并显示余额
3. **屏保显示** — 30 秒无操作后进入屏保，显示余额和时间
